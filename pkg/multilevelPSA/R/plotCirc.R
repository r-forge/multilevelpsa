plotcirc.multilevel.psa <- function(multilevelPSA,
	...
	p = p + geom_text(data=NULL, aes(x=multilevelPSA@plot.range[1], y=multilevelPSA@overall.mny, label=prettyNum(multilevelPSA@overall.mny, digits=2)), colour=overall.col, vjust=-.5, hjust=-.6, size=3) +
			geom_text(data=NULL, aes(y=multilevelPSA@plot.range[1], x=multilevelPSA@overall.mnx, label=prettyNum(multilevelPSA@overall.mnx, digits=2)), colour=overall.col, vjust=-.5, hjust=1.5, angle=-90, size=3)
	...
	p = p + geom_abline(slope=-1, intercept=(multilevelPSA@projection.intercept - .03 * diff(results.psa@plot.range)), colour='black', size=.5, alpha=.7)
	#Labels
...
setMethod('plotcirc', signature('multilevel.psa'), plotcirc.multilevel.psa)
